...
which literally states that $f^r$ lies in the ideal generated by the $f_i$.

Here is a slightly different proof of (N). 

It suffices to prove $I(V)\subset r(\mf a)$. 

(ZL) implies that $I(V)$ is the intersection of all the maximal ideals of $A:=k[x_1,\dots,x_n]$ containing $\mf a$. As $r(\mf a)$ is the intersection of all the prime ideals of $A$ containing $\mf a$, we can assume that $\mf a$ is a prime ideal $\mf p$. Let $f$ be in $A\setminus\mf p$. 

It suffices to find a maximal ideal of $A$ which doesn't contain $f$. 

Set $A_f:=A[1/f]$ and let $\mf p^{\oo e}$ be the extension of $\mf p$ in $A_f$. Clearly, $\mf p^{\oo e}$ is a proper ideal of $A_f$. Let $\mf m$ be a maximal ideal of $A_f$ containing $\mf p^{\oo e}$. 

Note successively that $f\notin\mf m$; that $\mf q:=\mf m\cap A$ is a prime ideal of $A$ which doesn't contain $f$; that $A_f/\mf m$ is isomorphic to $k$ by (ZL); and that the monomorphism of $k$-algebras $A/\mf q\mono A_f/\mf m$ implies that $\mf q$ is a maximal ideal of $A$.
