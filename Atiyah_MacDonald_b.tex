...
In the first paragraph after the proof of Lemma 10.1, it is written ``Two Cauchy sequences are equivalent if $x_\nu-y_\nu\to0$ in $G$''. The correct condition is ``the image of $x_\nu-y_\nu$ tends to $0$ in $G/H$, where $H$ is the intersection of all neighborhoods of $0$ in $G$''.
...
In the setting considered in the last paragraph of p.~102, let $C\subset G^{\mathbb N}$ be the group of Cauchy sequences (this is indeed easily seen to be a subgroup of $G^{\mathbb N}$), set $\widehat G:=\lim G/G_i$, and define $\varphi:C\to\widehat G$ by $\varphi(x)_i:=\pi_i(x_j)$ where $\pi_i:G\to G/G_i$ is the canonical projection and $j$ is a large enough integer depending on $i$. Then $\varphi$ is a well defined group epimorphism, and it induces an isomorphism $C/\!\!\sim\ \xrightarrow\sim\widehat G$.
